...
const DASHBOARD_PANELS = Object.freeze(["overview", "customizer", "notifications", "welcome"]);
...
function normalizeSettings(input) {
  ...
  const c = isObject(input.customizer) ? input.customizer : {}, n = isObject(input.notifications) ? input.notifications : {}, w = isObject(input.welcome) ? input.welcome : {}, card = isObject(w.card) ? w.card : {}, dm = isObject(w.dm) ? w.dm : {}, m = isObject(input.moderation) ? input.moderation : {}, r = isObject(input.roles) ? input.roles : {}, a = isObject(input.automation) ? input.automation : {}, p = isObject(input.profile) ? input.profile : {};
